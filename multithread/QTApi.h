...
API
for the QueenTest All series board
Created by zhangwenchao 2015/06/12
*/
...
// #define ONEMBCMDLEN 4
    ...
    typedef struct _tag_QTStorage
    {
      ...
      volatile unsigned long long GetDataCnt;
      ...
      void Reset (void)
      {
        ...
        memset(osWriteOP,0,PENDINGS*sizeof(OVERLAPPED));
    ...
    typedef struct _tag_QTIntermedium
    {
      ...
      double InputRange[INPUT_RANGE_NUM];
      int GainCode[INPUT_RANGE_NUM];
      unsigned int PosSlope[DAC_NUM];
      int gGPIODelayTapSel[4];
      unsigned char DelayTapVal[128];//Store all tap values
      int GainStepIdx[GAIN_STEP_NUM];
      ...
      void Reset (void)
      {
        ...
        memset(GainStepIdx,-1,GAIN_STEP_NUM);
      }
      _tag_QTIntermedium ()
      {
        Reset ();
    ...
    {
      //double ArrayZeroLevelDCOffset[DAC_NUM][GAIN_STEP_NUM];
      //double ArrayPGAGain[DAC_NUM][INPUT_RANGE_NUM];
      //double ArrayDACCoeff[DAC_NUM][GAIN_STEP_NUM];
      ...
      void Reset (void)
      {
        //memset(ArrayZeroLevelDCOffset,0,sizeof(double)*(DAC_NUM*GAIN_STEP_NUM));
        //memset(ArrayPGAGain,0,sizeof(double)*(DAC_NUM*INPUT_RANGE_NUM));
        //memset(ArrayDACCoeff,0,sizeof(double)*(DAC_NUM*GAIN_STEP_NUM));
    ...
    {
      unsigned long long SRate;                 //Sample rate in Hz
      unsigned long long Fclk;                  //Required frequency from clock board to ADC chip in Hz
      unsigned long long F_adclk;               //ad clock frequency in FPGA in Hz
    ...
    typedef struct _tag_QTVFIFO
    {
      ...
      volatile long long WrPtr,RdPtr,DataLenB;
      volatile long long CurWrPtr;
	    long long PERBUFLEN;//VFIFO size
	    long long FifoSizeB;   
	    volatile long long Dma_i;
	    long long ufill;//dummy variable, make memory align
    ...
    typedef struct _tagCardInfo
    {
        ...
        //long long llInstMemBytes;         // installed on-board memory in bytes
        ...
        //long long llSetMemsize;           // programmed memory size
        ...
        unsigned int SegmentSize;              //TODO:change to unsigned long long type
        ...
        struct ST_AI
        {
            ...
            double OffsetVoltVSCode[DAC_NUM];   //1.0:Pos-proportion; -1.0:Neg-proportion
            ...
            void Reset (void)
            {
                ...
                InputChannelMask = 0;
                DataFormat = 0;
                memset(OffsetVoltVSCode,1,DAC_NUM);
                bForceIOdelay = 0;
            }
            ST_AI ()
            {
  ...
  QTAPI int __stdcall nQTSetupTrigSoftware (ST_CARDINFO *pstCardInfo,unsigned int unTrigType,unsigned int unEnable);
    ...
    //输入参数：
    //pstCardInfo：板卡信息块
    //unExtIdx：QTFM_COMMON_TRIGGER_SOURCE_  externAL_1 to QTFM_COMMON_TRIGGER_SOURCE_  externAL_13
    //unTrigType：QTFM_COMMON_TRIGGER_TYPE
    //unEnable：是否使能
    //返回值：
    //成功返回RES_SUCCESS，否则返回值在DRV_RESULT_CODE中定义
  ...
  QTAPI int __stdcall nQTSetupTrigExternal (ST_CARDINFO *pstCardInfo,unsigned int unExtIdx,unsigned int unTrigType,unsigned int unEnable);
    ...
    //输入参数：
    //pstCardInfo：板卡信息块
    //unChannel：channel to modify,Not selectable for now. Release this feature in next version,It is UI's responsibility to convert iChannel index to physical channel mask bit
    //unTrigtype：QTFM_COMMON_TRIGGER_TYPE
  ...
  extern "C"
#endif
  QTAPI int __stdcall nQTSendData (ST_CARDINFO *pstCardInfo, void *pBufSrc, unsigned int unLen, unsigned int unTimeOut/*=0xFFFFFFFF*/);
...
//RangeVolt:通道输入范围选择，取值范围0~(N-1)，其中N是板卡支持的输入范围档位个数，分别对应1X,2X,5X,10X放大。
  ...
  extern "C"
#endif
  QTAPI int __stdcall nQTBufStatusGet (ST_CARDINFO *pstCardInfo,long long  *pDataLenB,int *pBufOverflow,unsigned int *pNoBytesOverwritten);
