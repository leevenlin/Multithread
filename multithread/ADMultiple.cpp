...
#include "math.h"
extern unsigned char *totalDataBuf;
...
int QT_FifoMultiple(ST_CARDINFO *pstCardInfo, unsigned int unBoardIndex, paramater pmt)
{
	...
	int BufOverflow = -1;//buffer overflow flag
	unsigned int NoBytesOverwritten = 0;//No. of overwritten sample data bytes
	unsigned long long TotalData = (unsigned long long) 1 << 29; //512MB

	unsigned int StartAngleFileName = pmt.angleStart * 4096 / 360;
	unsigned int EndAngleFileName = pmt.angleEnd * 4096 / 360;
	unsigned int SegmentLenTmp = Range2Byte(pmt.sampleRange);
	unsigned int SegmentLenFileName = SegmentLenTmp << 10 - 16;//number of samples per segment
	bool         SaveFile = pmt.saveFile;
	
	int nRet = -1;//Default return value
	unsigned char *pszBuf = NULL;
	unsigned int PerBufSize;
	PerBufSize = 64 << 20;//100MB,should be aligned with pstCardInfo->dib.EachTransferLen_Tx
	pszBuf = (unsigned char*)malloc(sizeof(char)*PerBufSize);
	...
	while (1)
	{
		...
		sprintf_s(filename, MAX_PATH, "%d-%d-%d-%d-%d-%d-%d-%d-%d.dat", timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec, SegmentLenFileName, StartAngleFileName * 360 / 4096, EndAngleFileName * 360 / 4096);
